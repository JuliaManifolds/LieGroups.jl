...
\begin{figure}
    ...
    % Define arrow length
    \def\arrowlength{0.15}

    % Load and iterate over CSV data
    \pgfplotstableread[col sep=comma]{data/bezier_example_se2.csv}\datatable

    % Iterate over rows
    \pgfplotstableforeachrow{\datatable}{%
        \pgfplotstablegetelem{\pgfplotstablerow}{x}\of{\datatable}%
        \pgfmathsetmacro{\x}{\pgfplotsretval}%
        \pgfplotstablegetelem{\pgfplotstablerow}{y}\of{\datatable}%
        \pgfmathsetmacro{\y}{\pgfplotsretval}%
        \pgfplotstablegetelem{\pgfplotstablerow}{u}\of{\datatable}%
        \pgfmathsetmacro{\u}{\pgfplotsretval}%
        \pgfplotstablegetelem{\pgfplotstablerow}{v}\of{\datatable}%
        \pgfmathsetmacro{\v}{\pgfplotsretval}%

        % Normalize vector
        \pgfmathsetmacro{\norm}{sqrt(\u*\u + \v*\v)}%
        \pgfmathsetmacro{\dx}{\arrowlength * \u / \norm}%
        \pgfmathsetmacro{\dy}{\arrowlength * \v / \norm}%

        % Draw arrow
        \draw[->, thick, blue] (\x,\y) -- ({\x+\dx},{\y+\dy});%
    }
    \end{axis}
\end{tikzpicture}
\end{figure}
