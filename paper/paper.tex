...
\begin{equation*}
    ...
    = \begin{pmatrix} \dot{g} \\ f(t, g, \dot{g}) \end{pmatrix},
\end{equation*}
where \(g \in \mathcal G_1\) and \(f(t, g, \dot{g})\) is derived from the above equations. We further require initial conditions for both \(g\) and \(\dot{g}\) at time \(t_0=0\).
...
# f((p,θ), (dp,dθ), t) including system parameters
function dynamics(state, sp::SystemParameters, t)
    p, dp = state.x
    F, tau = force_and_torque(sp, p.x...)
    a = F ./ sp.M
    coords = [tau / sp.Icm, a...]
    ddx = hat(se2, coords, ArrayPartition) #TODO se2 from global scope?
...
function integrator_step!(
        ...
        y, f, t, dt
) # TODO how are f and dynamics are they the same related?
    Ie = Identity(A.group)
    F1 = (dt / 2) * f(y, sp, t)
