...
\newcommand{\LieGroupsVersion}{0.1.9}
%
...
Several software packages for solving problems involving Lie groups exist, especially in the context of robotics and when solving differential equations or working with statistics on Lie groups.
These then often contain Lie groups as a part or sub-module.
Similar to the standalone packages like Lie++~\cite{FornasierGeGoorMahonyWeiss:2023}, Sophus~\cite{Strasdat:2024} in C++
or jaxlie~\cite{YiLeeKlossMartinMartinBohg:2021} in Python/JAX or manif~\cite{DeraySola:2020},
these mainly focus on a small set of Lie groups, mostly \(\mathrm{SO}(n)\) and \(\mathrm{SE}(n)\) for \(n = 2, 3\), sometimes also including simple product groups or the Gallilean group.

Two approaches that are a bit broader are the Lie groups defined within DiffMan~\cite{EngoeMarthinsenMunthe-Kaas:2001} in MATLAB to solve differential equations on manifolds,
and the Lie group implementations within the Python package geomstats~\cite{MiolaneGuiguiLeBrigantMatheHouThanwerdasHeyderPeltreKoepZaatitiHajriCabanesGeraldChauchatShewmakeBrooksKainzDonnatHolmesPennec:2020},
though they are a bit hard to explore within the documentation, since they are not distinguished from geometry interfaces, manifolds or metrics.

Formerly, the Julia package \texttt{Manifolds.jl}~\cite{AxenBaranBergmannRzecki:2023} contained some group operations, extending a few manifolds to Lie groups as well; the interface however ended up being too close to Riemannian features.

\texttt{LieGroups.jl} addresses this gap by providing both a flexible interface for defining new Lie groups and a comprehensive library of implementations, fully integrated within the \texttt{JuliaManifolds} ecosystem.
...
The following notation and definitions follow the text books~\cite{HilgertNeeb:2012},
especially Chapters 2 and 9 therein.
For more details on Riemannian manifolds, see also~\cite{DoCarmo:1992} or~\cite{Boumal:2023}, especially Chapter 8 therein.
...
A manifold \(\mathcal{M}\) is a topological space that locally looks like an Euclidean space \(\mathbb{R}^n\) for some \(n \in \mathbb{N}\), but globally may have a different topology.
We further require \(\mathcal{M}\) to be second-countable and Hausdorff, see~\cite[Sec.~8.2]{Boumal:2023} for details.

That a manifold locally looks like \(\mathbb{R}^n\) means that there exist a collection of \term{charts}
\(\varphi_i\colon U_i \to V_i \subseteq \mathbb{R}^n\) for open sets \(U_i \subseteq \mathcal{M}\) and \(V_i \subseteq \mathbb{R}^n\) such that the union of all \(U_i\) covers \(\mathcal{M}\).
Such a collection is called an \term{atlas} $\mathcal{A}$. If for any two charts \(\varphi_i,\varphi_j\) the transition map \(\varphi_j \circ \varphi_i^{-1}\colon \varphi_i(U_i \cap U_j) \to \varphi_j(U_i \cap U_j)\) is smooth,
then the atlas $\mathcal A$ is called a \term{smooth atlas} and the manifold $\mathcal M$ is called a \term{smooth manifold}.
We refer to \cite[Sec.~8.1]{Boumal:2023} for more details on charts and atlases.
...
which locally looks like \(\mathbb{R}^2\), the corresponding charts $\varphi_i$ are what you would expect in a classical atlas,
but globally it is not homeomorphic to \(\mathbb{R}^2\).

Finally we denote the \term{tangent space} at a point \(p \in \mathcal{M}\) by \(T_p\mathcal{M}\).
This can be thought of as all “velocities” (direction and speed) in which a smooth curve $c(t)$ on the manifold can “pass through” the point $p = c(0)$ at time $t=0$.
Formally these velocities can be obtained by looking at the curve “through a chart” $\varphi\in\mathcal A$ and consider the derivative of $\varphi \circ c\colon \mathbb R \to \mathbb R^n$.
Since we consider smooth manifolds, this derivative can be defined independent of the chosen chart due to the smoothness of the transition maps.
Finally, the set of all “velocities” is set the equivalence classes of derivatives of these smooth curves~\cite[Sec.~8.4]{Boumal:2023}.
Each such tangent space $T_p\mathcal M$ is a $n$-dimensional vector space.
...
For the example of the $2$-dimensional sphere $\mathbb S^2$, the tangent space at a point $p \in \mathbb S^2$ is given by all vectors in $\mathbb R^3$ that are orthogonal to $p$,
or in other words the plane that is tangent to the sphere at $p$.
\\
...
To generate the identity in a specific representation by objects of type \verb|T|, one can use \verb|identity_element(G, T)|.
...
Evaluate the Beziér curve at parameter `t` given a
vector of control points `cp` on the Lie group `G`.
"""
