...
and their numerical aspects, for example when solving differential equations on Lie groups~\cite{Munthe-Kaas:1998,IserlesMunthe-KaasNoersettZanna:2000}.
...
This paper provides an overview of the main features of \verb|LieGroups.jl| as of version \LieGroupsVersion{}, whose logo is shown on the right.
%
...
The identity element is given by the identity matrix \(I\) (or the angle $\alpha=0$) and the inverse of a rotation matrix is given by its transpose $p^{\mathrm{T}}$ (or an angle $-\alpha$). % -(-\pi) would be out of the range
...
Using the differential $D\lambda_g(h)\colon T_h\mathcal G \to T_{g\cdot h}\mathcal G$, we can generate a so-called \term{left-invariant vector field} $\mathcal X(g) \coloneqq D\lambda_g(e)[X]$ which is uniquely determined by the choice of $X \in \mathfrak{g}$.%
...
For example \verb|identity_element(G)| returns the identity element $e$ of a Lie group $G$ in a default representation, \verb|identity_element!(G, e)| writes the result into the pre-allocated variable \verb|e|, which can also be used with other representation types. To generate a point in a specific representation of type \verb|T|, one can use \verb|identity_element(G, T)|.
...
.
If we want to “start walking” from another point, we can “move” (or interpret $X$) as being from the tangent space at some point $g$ and obtain by the chain rule the \term{exponential function} $\exp_g\colon\mathfrak g\to\mathcal G$ defined by $\exp_g(X)\coloneqq g \cdot \exp(X)$.
...
Given a vector $c \in \mathbb R^n$, we obtain the tangent vector by calling \verb|get_vector(|$\mathfrak g$\verb|, c, b)| where \verb|b| is a basis of the tangent space, i.e.\ a subtype of \verb|AbstractBasis| and defaults within \verb|LieGroups.jl| to \verb|DefaultLieAlgebraOrthogonalBasis()|.
...
To the representation of a tangent vector $X \in \mathfrak g$ to be represented in the tangent space at $g \in \mathcal G$ we have to use the push forward of the left multiplication with $g$, i.e.\ $D\lambda_g(e)[X]$. This is implemented as \verb|push_forward_tangent(G, g, X)|%
...
to compute \(D\sigma(g, p)[Y]\) for a Lie algebra element \(Y \in \mathfrak g\).
...
\begin{figure}[tbp]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            ymin = -3.25, ymax = 2.75,
            title style={yshift=-.5\baselineskip,},
            title={Trajectories of a charged rod between two point charges},
            xlabel={x [m]},
            ylabel={y [m]},
        ] %
