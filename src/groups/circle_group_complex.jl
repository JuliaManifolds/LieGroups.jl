...
function get_coordinates_lie(
    𝔤::LieAlgebra{𝔽,Op,CircleGroup{ℂ, Circle{ℂ}}}, X, ::DefaultLieAlgebraOrthogonalBasis{𝔾}
...
function get_coordinates_lie!(
    𝔤::LieAlgebra{𝔽,Op,CircleGroup{ℂ, Circle{ℂ}}}, c, X, ::DefaultLieAlgebraOrthogonalBasis{𝔾}
) where {𝔽,Op<:AbstractGroupOperation,𝔾}
    G = base_lie_group(𝔤)
    M = base_manifold(G)
...
function get_vector_lie(
    𝔤::LieAlgebra{𝔽,Op,CircleGroup{ℂ, Circle{ℂ}}},
    c,
...
function get_vector_lie!(
    𝔤::LieAlgebra{𝔽,Op,CircleGroup{ℂ, Circle{ℂ}}}, X::T, c, ::DefaultLieAlgebraOrthogonalBasis{𝔾}
...
identity_element(::CircleGroup{ℂ, Circle{ℂ}}) = 1.0 + 0.0im
identity_element(::CircleGroup{ℂ, Circle{ℂ}}, T::Union{<:Number,Type{<:Number}}) = one(T)
identity_element(::CircleGroup{ℂ, Circle{ℂ}}, ::Type{<:SArray{S,T}}) where {S,T} = @SArray fill(one(T))
identity_element(::CircleGroup{ℂ, Circle{ℂ}}, ::Type{<:MArray{S,T}}) where {S,T} = @MArray fill(one(T))
